...
class MarkdownGenerator:
    ...
    def _generate_api_summary(self, modules: list[ModuleDoc]) -> str:
        ...
        summary_groups = self._readme_summary_groups(modules)
        use_package_headings = len(summary_groups) > 1
        public_modules = [
            module for _, _, group_modules in summary_groups for module in group_modules
        ]
    ...
    def _readme_summary_groups(
        self,
        modules: list[ModuleDoc],
    ) -> list[tuple[str, str | None, list[ModuleDoc]]]:
        """Group README summary modules by package and surface package docstrings."""
        package_summaries: dict[str, str | None] = {}
        grouped_modules: dict[str, list[ModuleDoc]] = {}

        for module in sorted(modules, key=lambda m: (m.package, m.name)):
            if module.name == "__init__":
                package_summaries[module.package] = self._short_doc_line(module.docstring)
                continue
            grouped_modules.setdefault(module.package, []).append(module)

        return [
            (group_name, package_summaries.get(group_name), group_modules)
            for group_name, group_modules in grouped_modules.items()
        ]
    ...
    def generate_string(self, module: ModuleDoc) -> str:
        ...
        filtered_modules = self._filtered_modules([module])
        filtered_module = filtered_modules[0]
        type_index = TypeIndex.from_modules(filtered_modules)
        return self._render_module_content(filtered_module, type_index=type_index)

    def _filtered_modules(self, modules: list[ModuleDoc]) -> list[ModuleDoc]:
        """Return modules filtered by current member visibility options."""
        return filter_modules(modules, self._output_options)
